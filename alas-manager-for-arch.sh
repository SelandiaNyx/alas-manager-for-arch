...
# 检查环境并安装/更新 Dockerfile
setup_environment() {
    echo -e "${YELLOW}[!] 正在检查并更新基础环境...${NC}"
...
check_update() {
    if [ ! -d "$INSTALL_DIR/src" ]; then
        echo -e "${YELLOW}[!] 尚未检测到源码，执行首次克隆...${NC}"
        git clone "$REPO_URL" "$INSTALL_DIR/src"
        apply_patches
        return
    fi

    echo -e "${YELLOW}[*] 正在检查 ALAS 源码更新...${NC}"
    cd "$INSTALL_DIR/src"
    git fetch
    ...
    if [ $LOCAL != $REMOTE ]; then
        echo -e "${GREEN}[+] 发现新版本！正在更新源码...${NC}"
        git pull
        apply_patches
        echo -e "${YELLOW}[*] 源码已更新，建议稍后重新构建镜像。${NC}"
    else
        echo -e "${GREEN}[OK] ALAS 已经是最新版本。${NC}"
    fi
    cd "$INSTALL_DIR"
}
...
# 修复依赖补丁
apply_patches() {
    echo -e "${YELLOW}[*] 正在应用指南中的 Patch (requirements.txt)...${NC}"
...
while true; do
    print_banner
    echo -e "1. ${GREEN}启动 ALAS${NC} (后台运行)"
    echo -e "2. ${YELLOW}停止 ALAS${NC}"
    echo -e "3. ${BLUE}查看 运行日志${NC}"
    echo -e "4. 检查/下载 源码更新"
    echo -e "5. 重新构建 镜像 (源码更新后使用)"
    ...
    if [ -d "$INSTALL_DIR" ]; then
        STATUS=$(docker ps -q -f name=alas)
        if [ ! -z "$STATUS" ]; then
    ...
    case $choice in
        ...
        2)
            if [ -d "$INSTALL_DIR" ]; then
                ...
                echo -e "${YELLOW}已尝试停止容器。${NC}"
            fi
            sleep 2
            ;;
        3)
            if [ -d "$INSTALL_DIR" ]; then
                echo -e "${BLUE}正在显示实时日志 (按 Ctrl+C 退出日志查看)...${NC}"
